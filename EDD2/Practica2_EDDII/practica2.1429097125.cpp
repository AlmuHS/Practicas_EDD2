...
int posicionInorden(const ABB<T> &a, const typename ABB<T>::Iterador& r, int num, int pos){
	if(!r.arbolVacio()){
		if(num < r.observar()) return posicionInorden(a, a.subIzq(r), num, pos + 1);
		else if(num > r.observar()) return posicionInorden(a, a.subDer(r), num, pos + 1 + numNodos(a, a.subIzq(r)));
		else if(num == r.observar()) return posicionInorden(a, r, num, pos);
	}
}
