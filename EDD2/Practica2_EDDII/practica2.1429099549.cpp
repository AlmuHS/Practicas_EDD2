...
int posicionInorden(const ABB<T> &a, int num){
	return posicionInorden(a, a.getRaiz(), num, 0);
}
...
int posicionInorden(const ABB<T> &a, const typename ABB<T>::Iterador& r, int num, int pos){
	if(!r.arbolVacio()){
		if(num < r.observar()) return 1 + posicionInorden(a, a.subIzq(r), num, 1);
		else if(num > r.observar()) return 1 + numNodos(a, a.subIzq(r)) + posicionInorden(a, a.subDer(r), num, 3);	
		else if(num == r.observar()) return 2 + posicionInorden(a, r, num, 2);
		/*else{
			if(num == a.subIzq(r).observar()) return 1;
			else if(num == a.subDer(r).observar()) return 3;
			else if(num == r.observar()) return pos 2;
		}*/
	}
	else return 0;
}
