	...
	if(dRecorrida < maxCoste){
		...
		while(!adyacentes.esVacio()){
			...
			if(dRecorrida < maxCoste && !Vrecorridos[vertice]){
				...
				caminosAcotados(G, ady.getObj(), maxCoste, dRecorrida + arista.getEtiqueta(), Vrecorridos, camino);
			}	
		}
	}
}
...
T outConectado(const Grafo<T, U>& G)
{
	...
	map<T, int> dvertices_out;
	map<T, int> dvertices_in;
	
	 while(!cvertices.esVacio()){
	 	Vertice<T> Vertice = cvertices.quitar();
		 dvertices_out[Vertice.getObj()] = 0;
		 dvertices_in[Vertice.getObj()] = 0;
	 }
	 
	 while(!caristas.esVacio()){
	 	Arista<T, U> arista = caristas.quitar();
	 	dvertices_in[arista.getDestino()] += 1;
	 	dvertices_out[arista.getOrigen()] +=1;
	 }
	 
	 
	for(typename map<T,int>::iterator it_in = dvertices_in.begin(); it_in != dvertices_in.end(); it_in++){
		if(it_in->second < dvertices_out[it_in->first])  return it_in->first;	
	}
}
